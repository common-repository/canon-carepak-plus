<?php
/*
Plugin Name: Canon CarePAK Plus
Plugin URI: https://www.afterinc.com/contact/
Description: Plugin to deals with product, creates and sell warranties for products.
Version: 2.2.6
Author: Canoncare
Author URI: https://www.afterinc.com
...
class Canoncarepack {
  ...
  public function __construct() {
    ...
    add_action( 'woocommerce_thankyou', array($this,'get_order_details' ));

   // add_filter( 'woocommerce_valid_order_statuses_for_cancel', array($this,'custom_valid_order_statuses_for_cancel'), 10, 2 );
...
/* apply filter for warranty and sync hook*/
  ...
  function add_js_customer() {
      ...
      wp_enqueue_script('contract_admin_js', plugin_dir_url(__FILE__) . 'assets/js/canoncare-customer.js');
      ...
      wp_enqueue_style( 'add_customer_css', plugins_url('/assets/css/canoncare-customer.css', __FILE__));
      wp_localize_script('jquery', 'ajax', array(
          'url' => admin_url('admin-ajax.php'),
          'nonce' => wp_create_nonce('Canoncarepack##Inc@20'),
      ));
  }
 ...
 /* add JS file for admin into the plugin*/
  function add_js_admin() {
      ...
      wp_enqueue_style('style_css', plugin_dir_url(__FILE__) . 'assets/css/admin/canoncare-style.css');
  ...
  function customplugin_menu() {
   add_menu_page("Custom Plugin", "CanonCarePak","manage_options", "CanonCarePak", array( $this, 'configuration_form' ), 'dashicons-format-aside');
   
   add_submenu_page( 'CanonCarePak', 'setting', 'Settings', 'manage_options', 'CanonCarePak', array( $this, 'configuration_form' ), 'dashicons-format-aside');

    add_submenu_page( 'CanonCarePak', 'Bulk Product Update', 'Bulk Product Update', 'manage_options', 'Bulk-Product-Update', array( $this, 'Submenu_function' ), 'dashicons-format-aside');
  }
...
/* 4th table create quickcover_contract */
  ...
  function get_currency_symbol(){
   ...
   /*6th table create product backup*/
...
/*Display custom warrenty filed data form database*/
   ...
   function sync_product_on_publish($new_status, $old_status, $post) {
    ...
    if( $old_status != 'publish' && $new_status == 'publish' && !empty($post->ID) && in_array( $post->post_type, array( 'product') )) {
      ...
      $dbData['sku'] = sanitize_text_field(wp_unslash($_POST['_sku'] !='' ? $_POST['_sku']: $_POST['post_ID']));
        ...
        $base_url = Canoncarepack::API_BASE_URL;
  ...
  function action_woocommerce_update_product($product_id ) { 
    ...
    if(!empty($result) && $result[0]->sync_type == 1 && $result[0]->configuration_property == 1 && $is_warrantyupdate == 'Enable'){
		...
		if (( $pagenow == 'post.php' ) || (get_post_type() == 'post')) {
			...
			else if($result[0]->configuration_property == 1 && $result[0]->warranty_status == 1 && $result[0]->sync_type == 0 && $warranty_type == 'New'){
              update_post_meta( $product_id, '_sync', 'New'); 
              }
  ...
  function afterinc_add_cron_interval($schedules){
                $schedules['my_oneminute_event'] = array(
                'interval' => 4*60*60,
                'display'  => esc_html__('scheduled my_oneminute_event'),
                );
                return $schedules;
        }
          ...
          ?>
              ...
              <div id="popupModel" class="modal1">
                ...
                <div class="modal-content1">
                  ...
                  <div>
                    <h3>After plan key benefits include:</h3>
                    <ul>
                      <li>Your After plan documents will be delivered to you via email only to the address associated with your order.</li>
                      <li>Fast, easy service including parts and labor, with no deductible for covered failures.</li>
                      <li>Nationwide network of service providers.</li>
                      <li>Power surge protection.</li>
                      <li>Accidental damage from Handling (AHD) coverage available on certain products.</li><a href="https://www.quickcover.me/after-plan" target="_blank">Learn More..</a>
                    </ul>
                    <ul></ul>
                  </div>
              ...
              <div class="parent-warranty-class">
                <span class="sortOptions">
                 ...
                 <div class="modal fade bd-example-modal-lg" id="planModal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" style='max-width:100%; width: 60%!important;'>
                    <div class="modal-dialog modal-lg">
                      ...
                      <div class="modal-content">
                        ...
                        <div class="modal-body">
                          ...
                          <iframe src="<?php echo esc_url_raw($productWrt[0]->learnMoreURL); ?>" width="100%" height="170" frameborder="0" sandbox="allow-same-origin allow-scripts allow-popups allow-forms allow-top-navigation" allowtransparency="true"></iframe>
                        </div>
                      </div>
                  ...
                  ?>
                    <input type="radio" id="war_<?php echo esc_attr($i); ?>" class="price-variation warranty-button" warranty-price="<?php echo esc_attr($seqPlans[$x]['price'] / 100); ?>" product-id="<?php echo esc_attr($product->get_id()) ?>" name="warranty" value="<?php echo esc_attr($seqPlans[$x]['planId']); ?>/<?php echo esc_attr($seqPlans[$x]['description']); ?>/<?php echo esc_attr($seqPlans[$x]['term']); ?>" plan-id="<?php echo esc_attr($seqPlans[$x]['planId']); ?>" <?php echo esc_attr($seqPlans[$x]['planId']) == $plan_id ? 'checked disabled' : '' ?>>
                ...
                <p class="trmcond"><a target="_blank" href="<?php echo esc_url_raw($result[0]->termsAndConditionsURL); ?>">Terms and Conditions</a></p>
              </div>
         <?php
  ...
  /*warranty price change on product description page */
    ...
    if(empty($result)){
      ...
      VALUES ( %d, %d, %d, %d, %s)",$current_user_id,$productId,$warrantyId,$warrantyPrice,$planId));
        ...
        $result = $this->wpdb->query( $this->wpdb->prepare( "UPDATE warranty_calc SET id = %d, user_id = %d, post_id = %d, warranty_id = %d, warranty_total = %s, plan_id = %s WHERE id = %d", $current_user_id, $productId, $warrantyId, $warrantyPrice, $planId, $id));
          ...
          $totalPrice1 = (float) $productActualPrice + (float) $warrantyPrice;
		  $totalPrice= sprintf('%.2f',$totalPrice1);
          echo esc_attr($totalPrice);
          exit();
    }
  }
  ...
  ?>
     <div class="admin-loader-div" style="display:none" id="loaderImg">
     <img src='<?php echo esc_url(plugin_dir_url( __FILE__ ))?>assets/images/Chasing_arrows.gif' id="loaderImg" >
   </div>
  <?php
  }
  ...
  function saveconfig(){
     ...
     if ( !wp_verify_nonce($_POST['nonce'], 'Canoncarepack##Inc@20') ){ 
     ...
     $current_date = sanitize_text_field($_POST['currentTime']);
      ...
      if ($response_status == 200) {
          $this->wpdb->query($this->wpdb->prepare("UPDATE app_configurations SET updated_date=%s,status=%s WHERE id= %d",$current_date,1, 1));
           ...
           plugin_log(['code'=>$response_status]); 
        } 
        else {
  ...
  function getBatch () {
    ...
    function syncProduct() {
      ...
      if ( $loop->have_posts() ): while ( $loop->have_posts() ): $loop->the_post();
        ...
        //$url = 'https://api.quickcover.me/v1/products/batch/outlet/'.$quick_store_id.'?upsert=true';
        ...
        if ($response_status == 202) {
          ...
          $failBatch = 1;
           foreach ($productList as $key => $value) {
            ...
            plugin_log(['code'=>$response_status,'response'=>$response,'request'=>$productJson]); 
          }
    ...
    if ( !wp_verify_nonce($_POST['nonce'], 'Canoncarepack##Inc@20') ){ 
     ...
     if($warranty_status !== "" && $sync_status !== ""){
        ...
        // Update status for sync
         ...
         //echo "<pre>"; print_r($sproducts); die;
      ...
      //$updtStatus = json_encode($updtStatus);
                ...
                if (!empty($cancel_result)) {
                   ...
                   $admin_login=$get_loginDetails[0]->user_login;
                    ...
                    if (!empty($_POST['orderID'] && !empty($_POST['lineItemID']))) {
                        ...
                        //echo "<pre>"; print_r($getContracts); die;
  ...
  function deleteProduct($product_id='') {
     ...
     if(!empty($result)){
      ...
      //$url = 'https://api.quickcover.me/v1/products/outlet/'.$storeId.'?productId='.$post_id;
 ...
 */
  ...
  function add_item_meta($item_data, $cart_item)
  {
    ...
    if(array_key_exists('warranty', $cart_item))
    {
       ...
       $warrantyData = $this->wpdb->get_results( $this->wpdb->prepare("SELECT * FROM product_warranty WHERE product_id=".$product_id));
      
      $pname = $warrantyData[0]->pname;
  ...
  function get_order_details($order_id) {
   ...
   $srlNo = count($order_items);
   $order_date = $order->get_date_created()->format('Y-m-d H:i:s');
   $orderItem = array(); $in_warranty = array();
   foreach ( $order_items as $key => $item ) {
    $lineitem_id = $item->get_id();
   ...
   $order_array = array(
      ...
      'id' => $order_data['id'],
       'customer'=> array(
        ...
        // $url = 'https://api.quickcover.me/v1/orders/outlet/'.$storeId;
       ...
       if($response_status==201){
        ...
        foreach ($contractData as $key => $value) {
         ...
         $currentID = get_current_user_id();
         $contrats = $this->wpdb->query(
         $this->wpdb->prepare("INSERT INTO quickcover_contract( user_id, product_id, order_id, lineitem_id, quantity, contract_id, contract_status, refund_amount, contract_canceled_by)
         VALUES ( %d, %d, %s, %s, %d, %s, %s, %s, %s)", $currentID , $value->order->product->id, $value->order->id, $value->order->orderItemId, $value->order->product->quantity, $value->id, 0, 0, 0 ));  
        ...
        $dataCron = $this->wpdb->insert('quickcover_cron_status', array(
  ...
  function custom_valid_order_statuses_for_cancel( $statuses, $order ){
    ...
    if(isset($_GET['order_id'])){
      ...
      $result = $this->wpdb->get_results( $this->wpdb->prepare( "SELECT * FROM quickcover_contract WHERE user_id = %d and order_id = %d", $user_id, $orderID ));
         ...
         if(!empty($result)){
           foreach ( $result as $res => $values ){
            ...
            $cancel_contract = array(
             ...
             // $url = 'https://api.quickcover.me/v1/contracts/outlet/'.$storeId.'/cancel';
              ...
              if($response_status == 200) {
                  ...
                  $rfamount = ($contract_responce['contracts'][0]['refundAmount']/100);
                 $contractUpdate = $this->wpdb->query( $this->wpdb->prepare( "UPDATE quickcover_contract SET contract_status = %s, refund_amount = %s, contract_canceled_by = %s WHERE contract_id = %s", $response_status, $rfamount, 'Customer', $contractID ));
    ...
    $duration = 3; // 3 days
   if( isset($_GET['order_id']))
    $order = wc_get_order( absint(sanitize_text_field($_GET['order_id']) ) );
  ...
  $item_warrantys = $item->get_meta_data();
     ...
     $quickCover_status = $get_configuration_property[0]->configuration_property;
	  ...
	  $warrantyData = $this->wpdb->get_results( $this->wpdb->prepare("SELECT * FROM product_warranty WHERE product_id=".$product_id));
      $pnamecustomer = $warrantyData[0]->pname;
...
if(isset($result[0]->contract_status) && $result[0]->contract_status == '200' && $result[0]->refund_amount !=''){
  ...
  $refundamtqty = $result[0]->refund_amount * $item['quantity'];
   ...
   $cancelButton = "<button class='cancelContract' id=".$item_quantity." style='display: block; padding: 5px 10px; font-size: 13px; border: none; background: #009688; font-family: sans-serif; float: right; margin-right: 320px' data-orderId=".$productOrderId." data-lineItemID=".$lineItemId." data-cancelby='customer'> Cancel ".$pnamecustomer."</button>";
...
function woocommerce_before_order_itemmeta($item_id, $item, $product){
  ...
  $quickCover_adminStatus = $get_configuration_admin[0]->configuration_property;
    $productOrderId = $item['order_id'];
    $order = wc_get_order( $productOrderId );
    $order_status = $order->get_status();
    $lineItemId = $item->get_id();
    ...
    $item_warrantys = $item->get_meta_data();
	 ...
	 $warrantyData = $this->wpdb->get_results( $this->wpdb->prepare("SELECT * FROM product_warranty WHERE product_id=".$product_id));  
     $pnameadmin = $warrantyData[0]->pname;
...
// Check this is the products screen
  ...
  $Getwarranty_selected = sanitize_text_field($_GET['warranty']);
  ...
  // Add your filter input here. Make sure the input name matches the $_GET value you are checking above.
	...
	$selectedNo =($Getsync_selected=='Failed')?'Selected':'';
...
// Check this is the products screen
    ...
    $Getsync_selected = sanitize_text_field($_GET['sync']);
...
// Ensure it is an edit.php admin page, the filter exists and has a value, and that it's the products page
 ...
 //echo '<pre>';print_r($meta_key_query);echo '</pre>';
...
public function addCustomImportButton()
{
   ...
   function my_phpmailer_afterinc($phpmailer){
    ...
    //cron code start here
     ...
     {
         plugin_log("This is testing for cron job every 4 hour");
...
if ( ! function_exists( 'plugin_log' ) ) {
  ...
  global $afterWarranty;
  $afterWarranty = new Canoncarepack();
}
