...
jQuery(document).ready(function(){
              ...
              success: function(resp) {
              	if(resp !=''){
                      ...
                      if( re.res !== "" && re.status == '200'){
                          ...
                          swal({
                                title: "Success",
                                text: "Contract has been cancelled successfully, We have initiated the refound refund of $"+total,
                                icon: "success",
                                button: "Ok",
                              }).then((value) => { 
	  ...
	  if(jQuery(this).is(":checked")){
		...
		var variationId = jQuery('input.variation_id').val();
			...
			success: function(response) {
			  ...
			  if(exist.length){
				jQuery('.woocommerce-variation-price .price ins .woocommerce-Price-amount bdi').text("$"+response);
			  }else{
				jQuery('.price .woocommerce-Price-amount bdi').text("$"+response);
			  }
 ...
 window.onload = function() {
    ...
    var total =  parseFloat(productPrice) + parseFloat(warrantyPrice);
	   if(total > 0){
		 var exist = jQuery('.woocommerce-variation-price .price ins .woocommerce-Price-amount bdi');
		  if(exist.length){
